...
(defencoder :utf-16be (src src-offset dst dst-offset length limit)
  (macrolet ((set-16 (offset value)
               `(setf (sb-impl::sap-ref-16be dst (+ k ,(* 2 offset))) ,value)))
    (let ((limit-4 (- limit 4))
          (length length))
      (declare (index limit-4 length))
      (do ((i 0 (1+ i))
...
(defdecoder :utf-16be (src src-offset dst dst-offset length limit)
  (do ((i 0)
    ...
    (declare (index i j) (optimize (safety 0) (speed 3)))
    (set-char-code j
        (let ((bits (sb-impl::sap-ref-16be src (+ src-offset i))))
          (incf i 2)
          (cond
            ((or (<= #xdc00 bits #xdfff)
                 (<= #xfdd0 bits #xfdef)
            ...
            ((<= #xd800 bits #xdbff)
             (let ((next (if (> length (+ i 2))
                             (sb-impl::sap-ref-16be src (+ src-offset i 2))
                             (return-from utf-16be-decoder
                               (values i j t)))))
               (incf i 2)
               (unless (<= #xdc00 next #xdfff)
                 (utf-16-decoding-error :bad2 next))
