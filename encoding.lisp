...
(defun %encode (encoding string string-offset buf buf-offset count x code1 code2)
  (funcall (character-encoding-encoder encoding)
           string string-offset buf buf-offset count x code1 code2))
...
(defmacro defencoder (encoding (src src-offset dst dst-offset length limit eol-0 eol-1)
                      &body body)
  (let ((name (symbolicate encoding "-ENCODER")))
    `(progn
       (defun ,name (,src ,src-offset ,dst ,dst-offset ,length ,limit ,eol-0 ,eol-1)
           (declare (string ,src)
            (type (or (simple-array (unsigned-byte 8) (*)) system-area-pointer) ,dst)
            (index ,src-offset ,dst-offset ,length ,limit ,eol-0)
            (type (or index null) ,eol-1))
         (labels ((encode-to-sap (,dst)
                    (etypecase ,src
                      ((simple-array character (*)) (encode-from-string ,src ,dst))
                      ((simple-array base-char (*)) (encode-from-string ,src ,dst))))
                  (encode-from-string (,src ,dst)
                    (declare (system-area-pointer ,dst))
                    ,@body))
           (declare (inline encode-from-string))
           (if (sb-impl::vectorp ,dst)
    ...
    `(progn
       (defencoder ,encoding
         ...
         (with-eol-encoding (,eol-0 ,eol-1)
           (let ((,limit (if ,eol-1 (1- ,limit) ,limit)))
             (do ((,i 0 (1+ ,i))
                  (,j 0))
                 ((or (>= ,i ,length)
                      (>= ,j ,limit))
               ...
               (do-codes (,char-code (char ,src (+ ,src-offset ,i)))
